...
%%
RpoL = [0.5 0.5 0.5 0.5 0.5 0.5];  % RpoL is a row vector
...
A = ones(1,wellquantities); gas_provide = 10;   % the constraint of the sum of gas injection of each well
...
GOR(1) = 50; GOR(2) = 50; GOR(6) = 50; GOR(3) = 100; GOR(4) = 100; GOR(5) = 100;  % GOR(7) = 100;  % scf/STB
...
if plotfunctions_fmincon ~= 1 && plotfunctions_fmincon ~= 2 && plotfunctions_fmincon ~= 3 && plotfunctions_fmincon ~= 4 && ...
        plotfunctions_fmincon ~= 5 && plotfunctions_fmincon ~= 6
...
if objectmodel == 1
    ...
    UB_production = [xmax(objectmodel,1) xmax(objectmodel,2) xmax(objectmodel,3),xmax(objectmodel,4) xmax(objectmodel,5) xmax(objectmodel,6)];  %  xmax(objectmodel,7)  % constraint of gas injection of each well
    
    if solver == 1
        % use fmincon to solve optimizaiton distribution
        [xmax_total{objectmodel},ymin_total{objectmodel},exitflag(objectmodel,wellquantities+1),output{objectmodel,wellquantities+1},lambda,grad,hessian] = fmincon(@objfunc_production,...
    ...
    if solver == 2
        ...
        problem{objectmodel,wellquantities+1} = createOptimProblem('fmincon','objective',@objfunc_production,...
            'x0',x0,'lb',LB,'ub',UB_production,...
            'Aineq',A,'bineq',gas_provide,'nonlcon',@noncons,'options',options{objectmodel,wellquantities+1});
        [xmax_total{objectmodel},ymin_total{objectmodel},exitflag(objectmodel,wellquantities+1),output{objectmodel,wellquantities+1},allmins{objectmodel,wellquantities+1}] = run(gs,problem{objectmodel,wellquantities+1});
    end
...
if objectmodel == 2
    % use fmincon to solve optimizaiton distribution
    LB = [XL(1) XL(2) XL(3) XL(4) XL(5) XL(6) ]; % XL(7)
    UB_benefit = [xmax(objectmodel,1) xmax(objectmodel,objectmodel) xmax(objectmodel,3),xmax(objectmodel,4) xmax(objectmodel,5) xmax(objectmodel,6)];  % xmax(objectmodel,7)  % constraint of gas injection of each well
    
    if solver == 1
        [xmax_total{objectmodel},ymin_total{objectmodel},exitflag(objectmodel,wellquantities+1),output{objectmodel,wellquantities+1},lambda,grad,hessian] = fmincon(@objfunc_benefit,...
...
if objectmodel == 3
    ...
    LB = [XL(1) XL(2) XL(3) XL(4) XL(5) XL(6) ]; % XL(7)
    UB_production = [xmax(objectmodel-2,1) xmax(objectmodel-2,2) xmax(objectmodel-2,3),xmax(objectmodel-2,4) xmax(objectmodel-2,5) xmax(objectmodel-2,6) ]; % xmax(objectmodel-2,7)
    UB_benefit = [xmax(objectmodel-1,1) xmax(objectmodel-1,2) xmax(objectmodel-1,3),xmax(objectmodel-1,4) xmax(objectmodel-1,5) xmax(objectmodel-1,6) ]; % xmax(objectmodel-1,7)
    UB_mix = [xmax(objectmodel,1) xmax(objectmodel,2) xmax(objectmodel,3) xmax(objectmodel,4),xmax(objectmodel,5) xmax(objectmodel,6) ]; % xmax(objectmodel,7)
    
    if solver == 1
        [xmax_total{objectmodel-2},ymin_total{objectmodel-2},exitflag(objectmodel-2,wellquantities+1),output{objectmodel-2,wellquantities+1}] = fmincon(@objfunc_production,...
            x0,A,gas_provide,[], [], LB, UB_production,@noncons,options{objectmodel-2,wellquantities+1});
        ytolrealmax_production = -ymin_total{objectmodel-2};
        
        [xmax_total{objectmodel-1},ymin_total{objectmodel-1},exitflag(objectmodel-1,wellquantities+1),output{objectmodel-1,wellquantities+1}] = fmincon(@objfunc_benefit,...
            x0,A,gas_provide,[], [], LB, UB_benefit,@noncons,options{objectmodel-1,wellquantities+1});
        ytolrealmax_benefit = -ymin_total{objectmodel-1};
        
        [xmax_total{objectmodel},ymin_total{objectmodel},exitflag(objectmodel,wellquantities+1),output{objectmodel,wellquantities+1},lambda,grad,hessian] = fmincon(@objfunc_mix,...
            x0, A,gas_provide,[], [], LB, UB_mix,@noncons,options{objectmodel,wellquantities+1});
    end
    
    if solver == 2
        
        problem{objectmodel-2,wellquantities+1} = createOptimProblem('fmincon','objective',@objfunc_production,'x0',x0,...
            'lb',LB,'ub',UB_production,'Aineq',A,'bineq',gas_provide,'nonlcon',@noncons,'options',options{objectmodel-2,wellquantities+1});
        [xmax_total{objectmodel-2},ymin_total{objectmodel-2},exitflag(objectmodel-2,wellquantities+objectmodel-2),output{objectmodel-2,wellquantities+1},allmins{objectmodel-2,wellquantities+1}] = run(gs,problem{objectmodel-2,wellquantities+1});
        ytolrealmax_production = -ymin_total{objectmodel-2};
        
        problem{objectmodel-1,wellquantities+1} = createOptimProblem('fmincon','objective',@objfunc_benefit,'x0',x0,...
            'lb',LB,'ub',UB_benefit,'Aineq',A,'bineq',gas_provide,'nonlcon',@noncons,'options',options{objectmodel-1,wellquantities+1});
        [xmax_total{objectmodel-1},ymin_total{objectmodel-1},exitflag(objectmodel-1,wellquantities+1),output{objectmodel-1,wellquantities+1},allmins{objectmodel-1,wellquantities+1}] = run(gs,problem{objectmodel-1,wellquantities+1});
        ytolrealmax_benefit = -ymin_total{objectmodel-1};
        
        problem{objectmodel,wellquantities+1} = createOptimProblem('fmincon','objective',@objfunc_mix,'x0',x0,...
            'lb',LB,'ub',UB_mix,'Aineq',A,'bineq',gas_provide,'nonlcon',@noncons,'options',options{objectmodel,wellquantities+1});
        [xmax_total{objectmodel},ymin_total{objectmodel},exitflag(objectmodel,wellquantities+1),output{objectmodel,wellquantities+1},allmins{objectmodel,wellquantities+1}] = run(gs,problem{objectmodel,wellquantities+1});
    end
