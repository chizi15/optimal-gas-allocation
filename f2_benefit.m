function [ f ] = f2_benefit( t )
global n coe RpgL RpoL RpwL Po Pg Pw Pig;
y = 0;
for i = 1:n+1
    y = y - coe(2,i)*t^(n+1-i);            % between 'y' and 'coe' it must be '-', because ...
...
netgasproduction = -y*RpgL(2)/10000 + t - t;     % unit: 10e4 m3/d
if netgasproduction < 0
    error('RpgL(2) is so small that gas production of 2# well is smaller than gas injection of that');
else
    f = -( (-y*Po*RpoL(2)) + (netgasproduction*Pg) - (-y*Pw*RpwL(2)) - Pig*t );       % units of f, y, RpgL and t are...
