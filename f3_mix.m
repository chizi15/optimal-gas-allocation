...
f1 = 0;
for i = 1:n+1
    f1 = f1 - coe(3,i)*t^(n+1-i);            % between 'y' and 'coe' it must be '-', because ...
...
netgasproduction = -f1*RpgL(3)/10000 +t - t;     % unit: 10e4 m3/d
if netgasproduction < 0
    error('RpgL(3) is so small that gas production of 3# well is smaller than gas injection of that');
end
f2 = -( (-f1*Po*RpoL(3)) + (netgasproduction*Pg) - (-f1*Pw*RpwL(3)) - Pig*t );       % units of f, y, RpgL and t are...
...
%% 
f = -( w1*f1/ymin(1,3) + w2*f2/ymin(2,3) );   
if f >= 0
    disp('mix index of 3# well is negetive')
end
end