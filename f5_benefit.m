function [ f ] = f5_benefit( t )
global n coe RpgL RpoL RpwL Po Pg Pw Pig;
y = 0;
for i = 1:n+1
    y = y - coe(5,i)*t^(n+1-i);            % between 'y' and 'coe' it must be '-', because ...
...
netgasproduction = -y*RpgL(5)/10000 + t - t;     % unit: 10e4 m3/d
if netgasproduction < 0
    error('RpgL(5) is so small that gas production of 5# well is smaller than gas injection of that');
else
    f = -( (-y*Po*RpoL(5)) + (netgasproduction*Pg) - (-y*Pw*RpwL(5)) - Pig*t );       % units of f, y, RpgL and t are...
